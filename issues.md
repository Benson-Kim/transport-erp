...
- **P1 — `formatPercentage(value)` divides by 100**, but `marginPercentage` is already stored as a percentage (e.g. `18.5`). Passing `18.5` yields `1,850%`. Trace the call sites: services store `marginPercentage` as a percent number, so this helper double-scales unless callers pre-divide. Pick one convention and enforce it in the type.
...
### P1 — `ServiceDetail.tsx`: the margin-percent bug in the customer-facing detail + VAT display bug

- **`formatPercentage(service.costVatRate)` and `formatPercentage(service.saleVatRate)`** — VAT rate is stored as `21.00` (a percentage). `formatPercentage(21)` → **`2,100%`**. So the pricing breakdown shows "VAT (2,100%)". This is on the **service detail page a user prints/reviews** — a glaring, customer-visible error.
- **`formatPercentage(marginPercent)`** used twice (Badge + Margin % cell) where `marginPercent` is already `*100` → **inflated**. And the "Markup %" uses raw `.toFixed(2)` (correct), so within one card, markup is right and margin % is wrong.
...
- **`services/Header.tsx` (P1, misnamed + wrong project name):** The file is `// DashboardClientComponents.tsx` exporting `ServicesHeader`, `DashboardRefreshButton`, `DashboardErrorAlert`, `NewUserWelcome`, `PerformanceTip` — a grab-bag of dashboard client components living under `services/` and exported via the services barrel as `Header`. (1) **`NewUserWelcome` title "Welcome to Enterprise Dashboard!"** — the old project name leaking to end users (schema comment, email `enterprise-dashboard.com`, welcome email, now this). (2) `NewUserWelcome`/`PerformanceTip` route to **`/import`** and imply an import feature that doesn't exist (dead route). (3) **`PerformanceTip` uses `formatPercentage(averageMargin)`** — ×100 bug, so the performance tip tells users their margin is e.g. "1,850%". (4) `DashboardErrorAlert` passes `icon={<Info/>}` to `Alert` whose `feedback-error-*` background is inert (styles) → error alert has no red background. Misfiled component (belongs in `dashboard/`), wrong name, dead route, wrong brand, and the margin bug.
...
- **P1 — `PerformanceTip` `formatPercentage(averageMargin)`** — ×100 inflation on the dashboard tip. _Confirms pattern: formatPercentage ×100._
...
- **`requireAuth()` server-side** — good, this is the real auth gate for the dashboard (compensating for the non-running middleware, partially). But `requireAuth` throws `'Unauthorized'` (auth.ts) which, uncaught in a layout, renders the Next error page, not a redirect to `/login`. So an unauthenticated dashboard hit shows an error, not a clean login redirect. _Confirms pattern: middleware-not-wired → auth UX degraded._
...
- **`formatPercentage` ×100:** StatsCard, MiniStats, ServicesTable, ServiceRow, ServicesMobileView, ServiceDetail (incl. VAT rates as "2100%"), PricingCalculator, PerformanceTip (×2 copies), ServiceForm. Charts avoid it. → split into two typed helpers.
...
- **Crash/build bugs:** `RecentServices` `next/router` import (App Router crash), `UserManagement` `usePermissions()` in `useCallback` (Rules of Hooks), `ServiceChart` dynamic `bg-${trendColor}` (purged), `prisma.ts` missing import + `signInWithCredentials` undefined `result` (v1), `getUser` `findUnique` non-unique where (v2), `revalidateTag('dashboard','default')` no-op, `_count._all` cast → dashboard always "new user", missing `middleware.ts` (proxy.ts) → root unprotected, empty `auth.config.ts`.
...
- **P1 — `pathname` from `headers().get('x-pathname')`** — that header is set by **`proxy.ts` middleware**, which isn't running as `middleware.ts` (v1 P0). So `x-pathname` is **always empty** → `isActive = pathname === href` is always false → **no settings nav item ever shows as active**. Confirms the middleware-not-wired issue has downstream functional impact beyond auth. _Confirms pattern: middleware not wired → `x-pathname` header absent._
...
- **`x-pathname` header empty** (middleware not wired) → settings nav never shows active.
...
- **P1 — `formatPercentage(client.stats.averageMarginPercentage)}%`** — double bug: `averageMarginPercentage` is already a percent (client-actions computes `sum(marginPercentage)/count`), `formatPercentage` divides by 100 AND appends `%`, then the JSX appends **another literal `%`** → renders e.g. `18.5%` as `"1850%%"` (Intl percent format outputs `1,850%`, plus the trailing `%` = `1,850%%`). Worst instance of the formatPercentage bug — double-scaled _and_ double-`%`. _Confirms pattern: formatPercentage ×100 (+ literal % here)._
...
- **P1 — `service.marginPercentage.toFixed(1)` — CORRECT here** (raw `.toFixed`, no `formatPercentage`). So this component shows margin % right, while `ClientDetail`'s sidebar (same page) shows it double-scaled. **Two margin-% renderings on the same client detail page disagree.** _Confirms pattern: inconsistent formatPercentage usage (chart/inline correct, helper wrong)._
