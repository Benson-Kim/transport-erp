/**
 * Service data access (server-only, not a 'use server' action module).
 *
 ...
 * double-fetching. Every read is gated with requireServiceAccess so an
 * ungated fetch cannot leak service data (e.g. via <title>).
 */
...
export type ServiceWithDetails = Awaited<ReturnType<typeof getServiceWithDetails>>;
