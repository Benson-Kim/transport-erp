...
class StorageService {
  ...
  private async validateFile(
    ...
    if (fileType && !restrictions.mimeTypes.includes(fileType.mime)) {
      throw new FileValidationError(`File MIME type ${fileType.mime} is not allowed`);
    }
  }
   ...
   */
  private async processImage(buffer: Buffer, options?: UploadOptions['resize']): Promise<Buffer> {
  ...
  public async uploadFile(
    ...
    try {
      ...
      if (error.message?.includes('ECONNRESET') || error.message?.includes('ETIMEDOUT')) {
        throw new FileUploadError('Connection to storage service was lost. Please try again.', {
          originalName,
          error,
        });
      }
      ...
      throw new FileUploadError(`Failed to upload file: ${error.message || 'Unknown error'}`, {
        originalName,
        error,
      });
    }
   ...
   */
  public async uploadLargeFile(filePath: string, options: UploadOptions = {}): Promise<FileInfo> {
    try {
      // Get file stats
      ...
      // Determine content type
      const contentType =
        options.contentType || mime.lookup(filePath) || 'application/octet-stream';
  ...
  public async getFileInfo(key: string): Promise<FileInfo> {
    try {
      ...
      throw new StorageError(`Failed to get file info: ${error.message}`, 'FILE_INFO_FAILED', 500, {
        key,
        error,
      });
    }
  ...
  public async deleteFile(key: string): Promise<void> {
    try {
      ...
      throw new StorageError(`Failed to delete file: ${error.message}`, 'FILE_DELETE_FAILED', 500, {
        key,
        error,
      });
    }
  ...
  public async copyFile(sourceKey: string, destinationKey: string): Promise<FileInfo> {
    try {
      ...
      throw new StorageError(`Failed to copy file: ${error.message}`, 'FILE_COPY_FAILED', 500, {
        sourceKey,
        destinationKey,
        error,
      });
  ...
  public async listFiles(options: ListFilesOptions = {}): Promise<{
    ...
    try {
      ...
      throw new StorageError(`Failed to list files: ${error.message}`, 'FILE_LIST_FAILED', 500, {
        options,
        error,
      });
    }
