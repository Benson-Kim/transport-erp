...
export class EmailService {
   ...
   */
  public async sendBatch(emails: EmailOptions[]): Promise<EmailSendResult[]> {
  ...
  public async processQueue(): Promise<{ processed: number; failed: number }> {
    ...
    for (const job of jobs) {
      const success = await this.processQueueJob(job as EmailQueueRecord);
  ...
  private resolveRecipients(to: string | string[]): string[] {
    ...
    if (Array.isArray(restrictions.testRecipients) && restrictions.testRecipients.length > 0) {
      this.log('info', `Redirecting emails to test recipients:`, {
        original: recipients,
    ...
    if (Array.isArray(allowedDomains) && allowedDomains.length > 0) {
      const filtered = recipients.filter((email) =>
        allowedDomains.some((domain) => email.toLowerCase().endsWith(domain.toLowerCase()))
      );
  ...
  private async processQueueJob(job: EmailQueueRecord): Promise<boolean> {
    try {
      ...
      const recipients = job.to.includes(',') ? job.to.split(',').map((s) => s.trim()) : job.to;

      const result = await this.sendTemplate(job.template as EmailTemplate, recipients, parsedData);
      ...
      if (currentAttempts < this.config.sending.maxRetries) {
        await prisma.emailQueue.update({
          ...
          data: {
            ...
            scheduledAt: new Date(Date.now() + this.config.sending.retryDelay * currentAttempts),
          },
        });
      } else {
        await prisma.emailQueue.update({
   ...
   */
  private async renderTemplate(template: EmailTemplate, data: EmailTemplateData): Promise<string> {
   ...
   */
  private getTemplateSubject(template: EmailTemplate, data: EmailTemplateData): string {
    ...
    const subjects: Record<EmailTemplate, () => string> = {
      ...
      invoice: () => `Invoice ${(data as InvoiceEmailData).invoiceNumber}`,
      'loading-order': () => `Loading Order ${(data as LoadingOrderEmailData).orderNumber}`,
