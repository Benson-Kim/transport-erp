...
export const companySettingsSchema = z.object({
  ...
  bankAccount: z
    ...
    .optional()
    .or(z.literal('')),
  bankDetails: z.string().max(500, 'Bank details must be less than 500 characters').optional(),
  logo: z
    .union([
...
export const updateUserSchema = z
  ...
  .refine(
    (data) =>
      !data.password || data.password.trim() === '' || data.password === data.confirmPassword,
    {
      message: 'Passwords do not match',
      path: ['confirmPassword'],
    }
...
export const backupSettingsSchema = z.object({
  ...
  storageLocation: z.string().min(1, 'Backup location is required'),
  enabled: z.boolean(),
});
...
export const generalSettingsSchema = z.object({
  ...
  autoArchiveMonths: z.number().int().min(0, 'Cannot be negative').max(120, 'Maximum 120 months'),
});

// Combined system settings schema. numberSequences was deleted in #36: the
// tab wrote a setting nothing read - real numbering is the hardcoded
// document_counters allocator (src/lib/prisma/numbering.ts, #12).
