                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {resources.map(resource => {
                            const resourceActions = Object.keys(PERMISSION_MATRIX[resource as keyof typeof PERMISSION_MATRIX] || {});
                            ...
                            return resourceActions.map((action, actionIndex) => (
                                <tr
                                    ...
                                    className={cn(
                                        'border-b',
                                        actionIndex === 0 && 'border-t-2'
                                    )}
                                >
                                    {actionIndex === 0 && (
                                        <td
                                            rowSpan={resourceActions.length}
                                            className="p-2 font-medium bg-neutral-50"
                                        >
                                            <div className="flex items-center gap-2">
                                                <span className="capitalize">
                                                    {resource.replace(/_/g, ' ')}
                                                </span>
                                                <Tooltip content={PERMISSION_DESCRIPTIONS[resource as keyof typeof PERMISSION_DESCRIPTIONS]}>
