...
import { Button, Badge, Modal, DropdownMenu, Input, Select, DataTable, type Column } from '@/components/ui';
...
export function UserManagement({ users, currentUserId, currentUserRole }: UserManagementProps) {
    ...
    const relevantRoles = [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.MANAGER, UserRole.OPERATOR, UserRole.ACCOUNTANT, UserRole.VIEWER];
    const filteredByRole = users.filter(user =>
        relevantRoles.includes(user.role) || user.role === UserRole.SUPER_ADMIN
    );
    ...
    const filteredUsers = useMemo(() => {
        ...
        if (roleFilter !== 'all') {
            result = result.filter(user => {
                if (roleFilter === 'admin') return user.role === UserRole.ADMIN || user.role === UserRole.SUPER_ADMIN;
    ...
    const columns: Column<User>[] = useMemo(() => [
        ...
        },
        {
            ...
            header: 'Role',
            accessor: (row) => {
                const roleDisplay = row.role === UserRole.SUPER_ADMIN ? 'Admin' :
                    row.role === UserRole.ADMIN ? 'Admin' :
                        row.role === UserRole.MANAGER ? 'Manager' :
                            row.role === UserRole.OPERATOR ? 'Operator' : row.role;

                const variant = row.role === UserRole.SUPER_ADMIN || row.role === UserRole.ADMIN ? 'active' :
                    row.role === UserRole.MANAGER ? 'billed' : 'default';
    ...
    const handleBulkDeactivate = useCallback(async () => {
        const usersToDeactivate = selectedUsers.filter(id =>
            id !== currentUserId && filteredUsers.find(u => u.id === id && u.isActive)
        );
    ...
    return (
        <>
            ...
            <DataTable
                ...
                rowClassName={(row) => cn(
                    row.id === currentUserId && 'bg-neutral-50',
