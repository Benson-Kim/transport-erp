...
import { generateNumberPreview, parseFormatTokens, validateNumberFormat } from '@/lib/utils/number-format';
...
export default function SequenceSettings() {
    const { control, formState: { errors } } = useFormContext<SystemSettings>();
                ...
                {formatConfig.map((config) => (
                    <Controller
                        ...
                        render={({ field }) => {
                            ...
                            const preview =
                                previews[
                                config.name.replace('Format', '') as keyof typeof previews
                                ];
                            ...
                            return (
                                <FormField
                                    ...
                                    required
                                    error={
                                        errors.numberSequences?.[
                                            config.name as keyof typeof errors.numberSequences
                                        ]?.message ?? ''
                                    }
                                >
            ...
            <Controller
                ...
                render={({ field }) => (
                    ...
                    >
                        <Select
                            {...field}
                            options={RESET_FREQUENCY_OPTIONS}
