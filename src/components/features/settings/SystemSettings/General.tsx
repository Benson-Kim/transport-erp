...
export default function GeneralSettings() {
    const { control, watch, setValue, formState: { errors } } = useFormContext<SystemSettings>();

    const featureToggles = [
        { name: 'enableTwoFactor', label: 'Two-Factor Authentication', description: 'Require 2FA for all users' },
        { name: 'enableNotifications', label: 'Email Notifications', description: 'Send system notifications via email' },
        { name: 'enableAutoBackup', label: 'Automatic Backups', description: 'Run scheduled backups' },
        { name: 'requireClientVat', label: 'Require Client VAT', description: 'Make VAT number mandatory for clients' },
    ];
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <FormField
                                label="Date Format"
                                error={errors.general?.dateFormat?.message ?? ''}
                            >
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <FormField
                                label="Time Format"
                                error={errors.general?.timeFormat?.message ?? ''}
                            >
                                <Select {...field}
                                    className="w-full"
                                    size='md'
                            ...
                            render={({ field }) => (
                                ...
                                <FormField key={toggle.name}>
                                    ...
                                    <Switch
                                        ...
                                        onCheckedChange={(checked) => setValue(`general.${toggle.name}` as any, checked)}
                                    />
                                </FormField>
                            )}
                        />
