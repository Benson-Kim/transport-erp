...
export function ServiceTimeline({ serviceId }: ServiceTimelineProps) {
    ...
    const loadActivities = async () => {
        setIsLoading(true);
        try {
            const { activities: newActivities, hasMore: more } = await getServiceActivity(
                serviceId,
                { page: 1, limit: 10 }
            );
    ...
    const getActivityIcon = (action: string) => {
        switch (action) {
            case 'CREATE': return { icon: Plus, color: 'text-green-600' };
            case 'UPDATE': return { icon: Edit, color: 'text-blue-600' };
            case 'COMPLETE': return { icon: CheckCircle, color: 'text-green-600' };
            case 'CANCEL': return { icon: XCircle, color: 'text-red-600' };
            case 'GENERATE_DOCUMENT': return { icon: FileText, color: 'text-purple-600' };
            case 'SEND_EMAIL': return { icon: Mail, color: 'text-blue-600' };
            case 'ARCHIVE': return { icon: Archive, color: 'text-gray-600' };
