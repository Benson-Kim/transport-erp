    ...
    const renderItem = (item: NavItem, depth = 0): JSX.Element => {
        ...
        const content = (
            <button
                ...
                className={base}
            >
                <Icon className={cn('icon-sm)] flex-shrink-0', collapsed && variant === 'tablet' && 'icon-md)]')} />
        ...
        return (
            <li key={item.id}>
                {collapsed && variant === 'tablet' ? <Tooltip content={item.label} position="right">{content}</Tooltip> : content}
                {hasChildren && open && (!collapsed || variant === 'desktop') && (
                    <ul className="mt-space-1)] space-y-space-1">{item.children!.map(c => renderItem(c, depth + 1))}</ul>
                )}
            </li>
        );
    };
        ...
        >
            <div className="flex h-full flex-col">
                ...
                <header className="flex layout-header items-center justify-between border-b border-neutral-200 px-4">
                    {variant === 'tablet' && (
                        <button type="button" onClick={toggleSidebarCollapse} className="rounded-md p-1.5 hover:bg-row-hover">
