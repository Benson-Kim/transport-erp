...
export async function getSystemSettings(): Promise<SystemSettings> {
    ...
    const [email, pdf, backup, numberSequences, general] = await Promise.all([
        getSetting<EmailConfigInput>
            (SettingKey.EMAIL, DEFAULT_SYSTEM_SETTINGS.email),
        getSetting<PDFSettingsInput>
            (SettingKey.PDF, DEFAULT_SYSTEM_SETTINGS.pdf),
        getSetting<BackupSettingsInput>
            (SettingKey.BACKUP, DEFAULT_SYSTEM_SETTINGS.backup),
    ...
    switch (config.provider) {
        ...
        case 'ses': {
            ...
            try {
                ...
                throw new Error('Invalid AWS SES credentials. Expected JSON with accessKeyId, secretAccessKey, region.');
            }
...
async function executeBackup(_settings: BackupSettingsInput): Promise<BackupInfo> {
    ...
    try {
        ...
        if (stats.size > b2Config.maxFileSize) {
            throw new Error(`Backup size (${formatBytes(stats.size)}) exceeds maximum allowed size (${formatBytes(b2Config.maxFileSize)})`);
        }
...
export async function getLastBackupTime(): Promise<{
    ...
    const data = await getSetting<{
        ...
        filename: string;
        size: number;
    }>(SettingKey.LAST_BACKUP, null as unknown as { timestamp: string; filename: string; size: number });
 ...
 */
export async function verifyB2Configuration(): Promise<ActionResult<{ bucketName: string; endpoint: string }>> {
    try {
        await requirePermission('settings', 'view');
